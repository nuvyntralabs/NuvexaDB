...
public sealed partial class MainWindow : Window
{
    ...
    {
        var path = Path.Combine(AppContext.BaseDirectory, "winui.nvx");
        File.Delete(path);
        ...
        {
            await db.GetCollection("notes").InsertAsync(NuvexaDocument.Parse("""{"title":"hello from WinUI","n":1}"""));
            var rows = await db.ExecuteAsync("""db.notes.find({ n: { $gte: 1 } })""");
            await using var input = new MemoryStream("winui"u8.ToArray());
